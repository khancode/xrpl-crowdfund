...
int64_t hook(uint32_t reserved) {
    TRACESTR("Crowdfund Payment Hook (Development version)");
    ...
    if (mode_flag == MODE_CREATE_CAMPAIGN_FLAG) {
        ...
        return 0;
    } else if (mode_flag == MODE_INTEG_CREATE_CAMPAIGN_FLAG) {
        TRACESTR("Integ Mode: Create Campaign");
    ...
    } else if (mode_flag == MODE_INTEG_FUND_CAMPAIGN_FLAG) {
        TRACESTR("Integ Mode: Fund Campaign");
