...
export type HookOnTransactionType =
  | 'Payment'
  | 'EscrowCreate'
  | 'EscrowFinish'
  | 'AccountSet'
...
const ttMap = new Map<HookOnTransactionType, number>()
...
const calculateHookOn = (arr: HookOnTransactionType[]) => {
  ...
  arr.forEach((transactionType) => {
    ...
    // @ts-expect-error -- We know ttMap has n
    const ttNum: number = ttMap.get(transactionType)
    let v = BigInt(s)
